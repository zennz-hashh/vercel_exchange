...
export function Login({ onSwitch }: { onSwitch: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!ok) setError(t(user?.locale || 'id', 'auth.loginError'));
  };
  ...
  <div className="relative min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 flex flex-col items-center justify-center p-4 gap-8 overflow-hidden">
      {/* Remove stars background, keep planet and glowing circle */}
      ...
      {/* Remove old glowing circle, replaced by planet-bg */}
  <div className="relative z-10 bg-white rounded-2xl shadow-xl p-8 w-full max-w-md">
    {/* Focused glowing circle behind login card */}
    <div
      aria-hidden="true"
    ...
    />
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 text-sm">{t(user?.locale || 'id', 'app.tagline')}</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              {t(user?.locale || 'id', 'auth.email')}
            </label>
            <input
              type="email"
              value={email}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              {t(user?.locale || 'id', 'auth.password')}
            </label>
          ...
          <button
            type="submit"
            className="w-full bg-gradient-to-r from-blue-600 to-purple-600 text-white py-3 rounded-xl hover:from-blue-700 hover:to-purple-700 transition-all shadow-lg hover:shadow-xl font-semibold"
          >
            {t(user?.locale || 'id', 'auth.loginButton')}
          </button>
        </form>

        <p className="mt-6 text-center text-sm text-gray-600">
          {t(user?.locale || 'id', 'auth.dontHaveAccount')}{' '}
          <button onClick={onSwitch} className="text-blue-600 hover:text-blue-700 font-semibold">
            {t(user?.locale || 'id', 'auth.signup')}
          </button>
        </p>
      </div>
